...
describe('2 operators:', () => {
  ...
  it('12 operator every', () => {
    const firstResult = rx.getMessages(everyResult(rx.cold('a-b-c|', {a: 0, b: 1, c: 2})));
    const firstExpectation = rx.e('-----(a|)', {a: true});

    const secondResult = rx.getMessages(everyResult(rx.cold('a-b-c|', {a: 0, b: -1, c: -2})));
  ...
  it('29 operator exhaust', () => {
    ...
    const result = rx.getMessages(<any>exhaustResult(higherOrderObservable))
